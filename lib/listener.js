...
internals.Listener.prototype._publish = function (path, _update, options) {
    ...
    const each = async (socket, subscriberPath) => {       // Filter on path if has parameters

        let update = _update;

        // If we have a specific subscriber path (for wildcard matching), use it instead of the published path
        if (subscriberPath) {
            update = Object.assign({}, update);  // Shallow clone
            update.path = subscriberPath;
        }
    ...
    return route.subscribers._forEachSubscriber(match.paramsArray.length ? path : null, options, each, match);
};
...
internals.Listener.prototype._eachSocket = function (each, options) {
    ...
    return route.subscribers._forEachSubscriber(sub.paramsArray.length ? options.subscription : null, options, each, sub);       // Filter on path if has parameters
};
...
internals.Subscribers.prototype._pathMatches = function (subscriberPath, subscriberParams, publishedPath, publishMatch) {

    // Exact match case
    if (subscriberPath === publishedPath) {
        return true;
    }

    // For wildcard routes, check if subscriber path is a valid prefix
    if (publishMatch && publishMatch.params) {
        const subscriberParamsValue = subscriberParams.params;
        const publishedParamsValue = publishMatch.params.params;

        // Handle wildcard parameter matching
        if (subscriberParamsValue !== undefined && publishedParamsValue !== undefined) {
            // If subscriber has empty params, it should match everything
            if (subscriberParamsValue === '') {
                return true;
            }

            // Split the parameter values and check if subscriber is a prefix
            const subscriberSegments = subscriberParamsValue.split('/');
            const publishedSegments = publishedParamsValue.split('/');

            // Check if subscriber segments are a prefix of published segments
            if (subscriberSegments.length <= publishedSegments.length) {
                for (let i = 0; i < subscriberSegments.length; ++i) {
                    if (subscriberSegments[i] !== publishedSegments[i]) {
                        return false;
                    }
                }
        ...
        // Handle other parameter types (optional parameters, etc.)
        for (const paramName in subscriberParams) {
            const subscriberValue = subscriberParams[paramName];
            const publishedValue = publishMatch.params[paramName];

            // If subscriber has an empty optional parameter, it should match more specific ones
            if (subscriberValue === '' && publishedValue !== undefined) {
                return true;
            }
        }
    }
...
internals.Subscribers.prototype._forEachSubscriber = async function (path, options, each, publishMatch) {
    ...
    const itemize = async (item) => {
        ...
        if (!path) {
            // No path filtering, include all subscribers
            await each(item.socket);
            return;
        }

        // Check if any of the subscriber's paths match the published path
        for (let i = 0; i < item.paths.length; ++i) {
            const subscriberPath = item.paths[i];
            const subscriberParams = item.params[i];

            if (this._pathMatches(subscriberPath, subscriberParams, path, publishMatch)) {
                // Pass the subscriber's path to each so the message uses their subscription path
                await each(item.socket, subscriberPath);
                return; // Only call each once per subscriber
            }
        }
    };
